...
cmake -S "../kleidicv_rvv" -B "${BUILD_DIR}" \
  ...
  -DCMAKE_C_FLAGS="--target=riscv64-unknown-linux-gnu -march=rv64gcv -DGTEST_HAS_STEADY_CLOCK_=0" \
  -DCMAKE_CXX_FLAGS="--target=riscv64-unknown-linux-gnu -march=rv64gcv -DGTEST_HAS_STEADY_CLOCK_=0 -std=c++11" \
